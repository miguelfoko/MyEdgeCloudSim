/*The edge orchestrator module is the decision maker of the system. It uses 
 * the information collected from the other modules to decide how and where to
 *  handle incoming client requests. In the first version, we simply use a
 *   probabilistic approach to decide where to handle incoming tasks, but more
 *    realistic edge orchestrator can be added by extending abstract
...
public class AvnetEdgeOrchestrator extends EdgeOrchestrator{
...
//	private AvnetHelper avnetHelper;//Used for sliceScal
//	private final double latency_Required=0.17;//Used for sliceScal

	public AvnetEdgeOrchestrator(String _policy, String _simScenario) {
...
//		avnetHelper=new AvnetHelper();
	...
	public Vm getVmToOffload(Task task, int deviceId) {
		...
		if(deviceId == SimSettings.CLOUD_DATACENTER_ID){
			...
			List<Host> list = AvnetCoreSimulation.getInstance().getCloudServerManager().getDatacenter().getHostList();
	...
	public EdgeVM selectVmOnHost(Task task){
		...
		Location deviceLocation = AvnetCoreSimulation.getInstance().getMobilityModel().getLocation(task.getMobileDeviceId(), CloudSim.clock());
		...
		List<EdgeVM> vmArray = AvnetCoreSimulation.getInstance().getEdgeServerManager().getVmList(relatedHostId);
		...
		else if(policy.equalsIgnoreCase("SLICESCAL_FIT")){
			int tries = 0;
//			double t_total_fw=avnetHelper.sliceScal(latency_Required);
//			AvnetSimLogger.printLine("First:  CB_i= "+t_total_fw);
			while(tries < vmArray.size()){
				lastSelectedVmIndexes[relatedHostId] = (lastSelectedVmIndexes[relatedHostId]+1) % vmArray.size();
				double totalAvailableCPU=SimSettings.getInstance().getMipsForMobileVM();
				//double requiredCapacity = 10001;
				double requiredCapacity = ((CpuUtilizationModel_Custom)task.getUtilizationModelCpu()).predictUtilization(vmArray.get(lastSelectedVmIndexes[relatedHostId]).getVmType());
				double targetVmCapacity = (double)100 - vmArray.get(lastSelectedVmIndexes[relatedHostId]).getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
				//System.out.println("VmArray: "+ vmArray.size()+" MIPS for Mobile device: "+totalAvailableCPU+" TargetVmCapacity: "+targetVmCapacity);
				if(requiredCapacity <= targetVmCapacity){
					selectedVM = vmArray.get(lastSelectedVmIndexes[relatedHostId]);
	...
	public EdgeVM selectVmOnLoadBalancer(Task task){
		...
		if(policy.equalsIgnoreCase("RANDOM_FIT")){
			...
			List<EdgeVM> vmArray = AvnetCoreSimulation.getInstance().getEdgeServerManager().getVmList(randomHostIndex);
			int randomIndex = SimUtils.getRandomNumber(0, vmArray.size()-1);
			...
			for(int hostIndex=0; hostIndex<numberOfHost; hostIndex++){
				List<EdgeVM> vmArray = AvnetCoreSimulation.getInstance().getEdgeServerManager().getVmList(hostIndex);
				for(int vmIndex=0; vmIndex<vmArray.size(); vmIndex++){
					double requiredCapacity = ((CpuUtilizationModel_Custom)task.getUtilizationModelCpu()).predictUtilization(vmArray.get(vmIndex).getVmType());
					double targetVmCapacity = (double)100 - vmArray.get(vmIndex).getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
					if(requiredCapacity <= targetVmCapacity && targetVmCapacity > selectedVmCapacity){
			...
			for(int hostIndex=0; hostIndex<numberOfHost; hostIndex++){
				List<EdgeVM> vmArray = AvnetCoreSimulation.getInstance().getEdgeServerManager().getVmList(hostIndex);
				for(int vmIndex=0; vmIndex<vmArray.size(); vmIndex++){
					double requiredCapacity = 10;//((CpuUtilizationModel_Custom)task.getUtilizationModelCpu()).predictUtilization(vmArray.get(vmIndex).getVmType());
					double targetVmCapacity = (double)100 - vmArray.get(vmIndex).getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
					if(requiredCapacity <= targetVmCapacity){
						selectedVM = vmArray.get(vmIndex);
						break;
			...
			while(selectedVM == null && hostCheckCounter < numberOfHost){
				...
				List<EdgeVM> vmArray = AvnetCoreSimulation.getInstance().getEdgeServerManager().getVmList(lastSelectedHostIndex);
				while(tries < vmArray.size()){
...
//			AvnetSimLogger.printLine("First:  CB_i= "+t_total_fw);
			while(selectedVM == null && hostCheckCounter < numberOfHost){
				...
				while(tries < vmArray.size()){
					lastSelectedVmIndexes[lastSelectedHostIndex] = (lastSelectedVmIndexes[lastSelectedHostIndex]+1) % vmArray.size();
					double totalAvailableCPU=SimSettings.getInstance().getMipsForMobileVM();
					double requiredCapacity = 10001;//((CpuUtilizationModel_Custom)task.getUtilizationModelCpu()).predictUtilization(vmArray.get(lastSelectedVmIndexes[lastSelectedHostIndex]).getVmType());
					double targetVmCapacity = (double)100 - vmArray.get(lastSelectedVmIndexes[lastSelectedHostIndex]).getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
					//System.out.println("VmArray: "+ vmArray.size()+" MIPS for Mobile device: "+totalAvailableCPU+" TargetVmCapacity: "+targetVmCapacity);
					if(requiredCapacity <= targetVmCapacity){
						selectedVM = vmArray.get(lastSelectedVmIndexes[lastSelectedHostIndex]);
