        ...
        for i=1:size(scenarioType,2)
            for j=1:numOfMobileDevices
                try
                    ...
                    %filePath = strcat(folderPath,'\avnet',int2str(s),'\AVNET_SIMRESULT_',char(scenarioType(i)),'_BEST_FIT_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log');
                    %filePath = strcat(folderPath,'\avnet',int2str(s),'\AVNET_SIMRESULT_',char(scenarioType(i)),'_NEXT_FIT_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log');
                    filePath = strcat(folderPath,'\avnet',int2str(s),'\AVNET_SIMRESULT_',char(scenarioType(i)),'_SLICESCAL_FIT_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log');
                    disp(filePath);
